...
*** Updating dependencies along with the current project
  ...
  1. Checkout myproj: ~$ git clone https://github.com/myself/myproj $HOME/projects/myproj~
  2. Checkout mydep:  ~$ git clone https://github.com/myself/mydep $HOME/projects/mydep~
  3. Enter the nix-shell environment for myproj
     #+BEGIN_EXAMPLE
     $ nix-shell $HOME/my-devnix/configs.nix -A myproj.env
     [nix-shell] $
     #+END_EXAMPLE
  4. Create a ~cabal.project~ file that references mydep:
     #+BEGIN_EXAMPLE
  ...
  5. Normal dev and build:
     #+BEGIN_EXAMPLE
     $ edit myproj-files-or-mydep-files
     $ cabal v2-build
     $ cabal v2-test
     #+END_EXAMPLE
